...
const DoctorOrder = () => {
  ...
  console.log("doctorProfile", formData?.reference);
      ...
      <div className="flex flex-col rounded-3xl justify-center items-start">
        <main className="flex-1 bg-white rounded-3xl p-4 sm:p-6 w-full">
          <Formik
            ...
            // onSubmit={(v) => console.log("✅ Submit reached:", v)}
          >
            {({
              validateForm,
              values,
                        ...
                        <aside className="col-span-12 md:col-span-3 space-y-4">
                          ...
                          >
                            ...
                            <LabeledInput
                              ...
                              label="Office Reference number"
                              name="officeReg"
                              value={values.officeReg || ""}     // ✅ Formik controls the value
                              onChange={handleChange}
                              onBlur={handleBlur}
                              placeholder="Office Reference number"
                            />
                          ...
                          <MaterialDropdown
                            ...
                            value={values.scannerType || ""} // ✅ Controlled by Formik
                            onChange={(option) => {
                              if (!selectedTooth) {
                                dispatch(
                                  showToast({
                              ...
                              <FormSection className="p-0">
                                <DropdownWrapper buttonLabel="Modules">
                                  ...
                                  <MaterialDropdown
                                    ...
                                    className="w-full bg-white border px-4 py-3 text-sm text-textFieldHeading outline-none transition-shadow"
                                    options={
                                      orders.find((p) => p.name === "Denture")?.children || []
                                    }
                                    value={values.digitalOptions || ""} // ✅ use Formik state
                                    onChange={(option) => {
                                      if (!selectedTooth) {
                                        dispatch(
                                          showToast({
                                      ...
                                      // ✅ update Redux for tooth selection
                                      dispatch(
                                        updateToothSelection({
                                          toothId: selectedTooth,
                                          field: "digitalOptions",
                                  ...
                                  {/* <MaterialDropdown
                                    ...
                                    value={values.crown || ""}  // ✅ Formik state drives the UI
                                    onChange={(val) => {
                                      if (!selectedTooth) {
                                      ...
                                      // ✅ Update Redux/local state if needed
                                      handleDropdownChange("crown", {
                                        value: option?.value || val,
                                        label: option?.label || "",
                                        price: option?.price || 0,
                                      ...
                                      // ✅ Update Formik value so validation works
                                      setFieldValue("crown", option?.value || val);
                                    ...
                                    className="w-full bg-white px-4 py-3 text-sm text-textFieldHeading outline-none transition-shadow"
                                  // error={
                                  //   touched.crown && !values.crown
                                  //     ? "Please select a crown option"
                                  //     : ""
                                  // }
                                  /> */}
                                  ...
                                  <MaterialDropdown
                                    ...
                                    value={values.crown || ""} // ✅ Formik state
                                    onChange={(option) => {
                                      if (!selectedTooth) {
                                        dispatch(
                                          showToast({
                                      ...
                                      // ✅ Update Redux / local tooth selection
                                      dispatch(
                                        updateToothSelection({
                                          toothId: selectedTooth,
                                          field: "crown",
                                      ...
                                      // ✅ Update Formik value for validation + submission
                                  ...
                                  <MaterialDropdown
                                    ...
                                    value={values.material || ""} // ✅ Controlled by Formik
                                    onChange={(option) => {
                                      if (!selectedTooth) {
                                        dispatch(
                                          showToast({
                                      ...
                                      // ✅ Update Redux / local tooth state
                                      dispatch(
                                        updateToothSelection({
                                          toothId: selectedTooth,
                                          field: "material",
                                      ...
                                      // ✅ Update Formik state (for validation & submission)
                                      setFieldValue("material", option.value);

                                      // ✅ Mark as touched (optional, for error messages)
                                      setTouched((prev) => ({
                                        ...prev,
                                        material: true,
                                      }));
                                    ...
                                    storageKey="material"
                                  // error={
                                  //   touched.material && !values.material
                                  ...
                                  <MaterialDropdown
                                    className2="relative z-0"
                                    className="w-full bg-white px-4 py-3 text-sm text-textFieldHeading outline-none transition-shadow"
                                    options={orders.find((p) => p.name === "Digital Model Type")?.children || []}
                                    value={values.Model_type || ""} // ✅ Controlled by Formik
                                    onChange={(option) => {
                                      if (!selectedTooth) {
                                        dispatch(
                                          showToast({
                                      ...
                                      // ✅ Update Formik value for validation and submit
                                      setFieldValue("Model_type", option.value);

                                      // ✅ Mark as touched (optional, for Formik error display)
                                      setTouched((prev) => ({
                                        ...prev,
                                        Model_type: true,
                                      }));
                                    ...
                                    label="Digital Model Type"
                                    storageKey="Model_type"
                                  // error={
                                  //   touched.Model_type && !values.Model_type
                                  //     ? "Please select a digital model type"
                                  //     : ""
                                  // }
                                  />
                                  {/* 
                                  ...
                                  <MaterialDropdown
                                    ...
                                    value={values.lab || ""} // ✅ Controlled by Formik
                                    onChange={(option) => {
                                      if (!selectedTooth) {
                                        dispatch(
                                          showToast({
                                      ...
                                      // ✅ Update Formik value
                                      setFieldValue("lab", option.value);
                                    ...
                                    storageKey="lab"
                                  // error={
                                  //   touched.lab && !values.lab
                                  //     ? "Please select a dental lab alliance"
                                  //     : ""
                                  // }
                                  />
                                ...
                                <MaterialDropdown
                                  className2="relative z-0"
                                  className="w-full rounded-md bg-background border-none px-4 py-3 text-sm text-secondaryBrand outline-none transition-shadow"
                                  dropdownClass="text-secondaryBrand"
                                  options={
                                    orders.find((p) => p.name === "Photogrammetry files")?.children || []
                                  }
                                  value={values.photogrammetryfiles || ""} // ✅ Controlled by Formik
                                  onChange={(option) => {
                                    if (!selectedTooth) {
                                      dispatch(
                                        showToast({
                                    ...
                                    setTouched((prev) => ({
                                      ...prev,
                                      photogrammetryfiles: true,
                                    }));
                                  }}
                                  label="Photogrammetry Files"
                                  storageKey="photogrammetryfiles"
                                // error={
                                //   touched.photogrammetryfiles && !values.photogrammetryfiles
                                //     ? "Please select a photogrammetry file"
                                //     : ""
                                // }
                                />
