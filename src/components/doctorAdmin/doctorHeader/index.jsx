...
const DoctorHeader = ({ title, subTitle }) => {
  ...
  return (
    ...
    //   {/* )} */}
    // </div>
    <div className="flex justify-between items-center w-full px-4 py-4 md:px-6 md:py-6 ">
      {/* Left: Title / Subtitle */}
      <div className="flex flex-col justify-start items-start max-w-full">
        {title && !subTitle ? (
          <h1 className="font-poppins font-bold text-xl sm:text-2xl md:text-3xl leading-snug text-[#434343] truncate">
            {title}
          </h1>
        ) : title && subTitle ? (
          <div className="flex flex-col justify-start items-start">
            <p className="font-poppins font-normal text-xs sm:text-sm md:text-base text-[#434343] truncate">
              {title}
            </p>
            <h1 className="font-poppins font-bold text-xl sm:text-2xl md:text-3xl leading-snug text-[#434343] truncate">
              #{subTitle}
          ...
          <h1 className="font-poppins font-bold text-xl sm:text-2xl md:text-3xl leading-snug text-[#434343]">
            Place Order
          </h1>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-4 md:block hidden">
        {/* Notification / Icon */}
        <svg
          width="40"
          height="40"
        ...
        {/* User Info */}
        <div className="flex items-center gap-2 sm:gap-3 p-2 rounded-full bg-white">
          <img
            src="/assets/user.png"
            alt="user"
          ...
          <div className="flex flex-col justify-center">
            <p className="font-poppins font-semibold text-sm sm:text-base text-[#434343] truncate">
              Brainsim Hanry
            </p>
            <p className="font-poppins font-normal text-xs sm:text-sm text-[#949494] truncate">
              Hanry324@gmail.com
            </p>
          </div>
        </div>
